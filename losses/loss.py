...
class ClassLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true, seq_lens=None):
        with torch.no_grad():
            labels_classes = (y_true == 1).float()
        labels_classes = labels_classes.detach()   
        labels_classes.requires_grad = True
        return self.loss_fn(y_pred, labels_classes) * self.weight
