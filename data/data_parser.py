...
from config.config import PATH_TO_FEATURES, PATH_TO_LABELS, PARTITION_FILES
...
def fit_normalizer(task, feature, feature_idx=2):
    ...
    if task == 'reaction':
        ...
        df = pd.concat([pd.read_csv(training_csv)
                       for training_csv in tqdm(training_csvs)])
    else:
        df = pd.concat([pd.read_csv(training_csv)
...
def load_stress_subject(feature, subject_id, partition, emo_dim, normalizer, apply_segmentation=True,
    ...
    if not(normalizer is None):
        feature_data.iloc[:, -
                          feature_dim:] = normalizer.transform(feature_values)
    ...
    labels = []
    metas = []
    # each segment has columns: timestamp, segment_id, features, labels
...
def load_reaction_subject(feature, subject_id, normalizer, combined):
    ...
    try:
        feature_df = pd.read_csv(os.path.join(
            feature_path, f'{subject_id.replace("[","").replace("]","")}.csv'))
    except:
        return -1, -1, -1
    feature_idx = 2
